...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (router-first)\n"
            "For real user-submitted substantive prompts, prefer the universal `skill-router` "
            "CLI over Hermes' native installed-skill list. Run "
            "`skill-router preflight --json \"<latest user prompt>\"`; if it returns a clear "
            "route and the selected skill matches the user's core task, object, and action, "
            "load it with `skill-router skill <name>` through the terminal and follow the "
            "returned instructions. Do not call `skill_view` for a skill selected by "
            "`skill-router`, and do not treat router-selected skills as missing just because "
            "they are not installed in `~/.hermes/skills`. The router is the source of truth "
            "for universal skills.\n"
            "Use Hermes `skill_view(name)` only for native Hermes skills that are explicitly "
            "requested by name, for Hermes-agent maintenance, or when `skill-router` is "
            "unavailable and one of the local skills below clearly matches the task. Prefer "
            "no skill over a weak or generic local match.\n"
            "Whenever the user asks you to configure, set up, install, enable, disable, modify, "
            "or troubleshoot Hermes Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the native `hermes-agent` "
            "skill first with `skill_view(name='hermes-agent')`. It has the actual commands "
            ...
            "</available_skills>\n"
            "\n"
            "Only load a local Hermes skill when it is clearly relevant under the rules above."
        )
